...
def calculate_reward_with_varying_features(test_traj, policy_net, discrim_net, env):
    ...
    # Iterate through different values for the first, second, and fourth elements of path_feature
    for path_feature_fourth_value in np.arange(-1.0, 1.1, 0.2):  # 11 values
        for path_feature_first_value in np.arange(-1.0, 1.1, 0.1):  # 21 values
            for path_feature_second_value in np.arange(-1.0, 1.1, 0.1):  # 21 values
                # Create a new path_feature tensor with the modified values
                path_feature = original_path_feature.clone()
                path_feature[0] = path_feature_first_value
                path_feature[1] = path_feature_second_value
                path_feature[3] = path_feature_fourth_value
                ...
                reward_data.append({
                    ...
                    'path_feature_fourth_value': path_feature_fourth_value,
                    'reward': reward.item(),
                })
...
# Calculate rewards for varying path_feature first, second, and fourth values
reward_df = calculate_reward_with_varying_features(test_trajs, policy_net, discrim_net, env)

# Create heatmaps
fig = make_subplots(rows=3, cols=4, shared_xaxes=True, shared_yaxes=True, 
                    vertical_spacing=0.05, horizontal_spacing=0.02, 
                    subplot_titles=[f"#Right Turn: {v:.1f}" for v in np.arange(-1.0, 1.1, 0.2)])

# Create 11 heatmaps (one for each unique fourth value)
for i, fourth_value in enumerate(np.arange(-1.0, 1.1, 0.2)):
    row = i // 4 + 1
    col = i % 4 + 1
    
    df_slice = reward_df[reward_df['path_feature_fourth_value'] == fourth_value]
    
    # Sort the dataframe to make rewards increase from -1 to 1
    df_slice = df_slice.sort_values(['path_feature_first_value', 'path_feature_second_value'])
    
    df_pivot = df_slice.pivot(index='path_feature_first_value', columns='path_feature_second_value', values='reward')
    ...
    fig.add_trace(
        go.Heatmap(
            ...
            y=df_pivot.index,
            colorscale='Inferno',
            zmin=reward_df['reward'].min(),
            zmax=reward_df['reward'].max(),
            hovertemplate='#Link: %{y}<br>Distance: %{x}<br>Reward: %{z:.4f}<extra></extra>'
        ),
        row=row, col=col
    )
...
for i in range(11):
    fig.update_xaxes(title_text="Distance", row=(i//4)+1, col=(i%4)+1, tickfont=dict(size=8))
    fig.update_yaxes(title_text="#Link", row=(i//4)+1, col=(i%4)+1, tickfont=dict(size=8))

# Add a common colorbar
fig.update_layout(coloraxis=dict(colorbar=dict(title="Reward", len=0.75, y=0.5)))

# Show the plot
fig.show()

# Optionally, save to HTML
fig.write_html("./output/reward_heatmaps_1_2_4_original.html")
print("Saved heatmaps to ./output/reward_heatmaps_1_2_4_original.html")
...
#                 # Reverse the reward
#                 reversed_reward = -reward.item()

#                 reward_data.append({
#                     'path_feature_first_value': path_feature_first_value,
#                     'path_feature_second_value': path_feature_second_value,
#                     'path_feature_fourth_value': path_feature_fourth_value,
#                     'reward': reversed_reward,
#                 })
...
# fig = make_subplots(rows=3, cols=4, shared_xaxes=True, shared_yaxes=True, 
#                     vertical_spacing=0.05, horizontal_spacing=0.02, 
#                     subplot_titles=[f"Fourth Value: {v:.1f}" for v in np.arange(-1.0, 1.1, 0.2)])
...
# # Update x and y axis labels
# for i in range(11):
#     fig.update_xaxes(title_text="Second Value", row=(i//4)+1, col=(i%4)+1, tickfont=dict(size=8))
#     fig.update_yaxes(title_text="First Value", row=(i//4)+1, col=(i%4)+1, tickfont=dict(size=8))
...
# # Optionally, save to HTML
# fig.write_html("./output/reward_heatmaps_1_2_4_reversed.html")
# print("Saved heatmaps to ./output/reward_heatmaps_1_2_4_reversed.html")
...
# # Update x and y axis labels
# for i in range(11):
#     fig.update_xaxes(title_text="Second Value", row=(i//4)+1, col=(i%4)+1)
#     fig.update_yaxes(title_text="First Value", row=(i//4)+1, col=(i%4)+1)
...
# # Optionally, save to HTML
# fig.write_html("./output/reward_heatmaps_1_2_4.html")
# print("Saved heatmaps to ./output/reward_heatmaps_1_2_4.html")